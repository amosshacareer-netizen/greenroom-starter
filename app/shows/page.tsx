...
import { quickCheckAmountMismatch } from "@/lib/dealMath";
...
export default async function ShowsPage() {
  ...
  const serialized: ShowRow[] = reversed.map(({ show, artist, deal, settlement }) => {
    const parsed = deal?.dealNotesFreetext
      ...
      : null;
    const flags = parsed && deal
      ? generateWarnings(parsed, deal, [])
      : [];

    const hasDataConflict = flags.some(
      (w) =>
        w.type === "percentage_mismatch" ||
        w.type === "deal_type_mismatch" ||
        w.type === "basis_mismatch" ||
        w.type === "guarantee_mismatch"
    );
    const hasComplexDeal = flags.some((w) => w.type === "complex_deal");
    const hasAmountMismatch = deal && settlement
      ? quickCheckAmountMismatch(deal, settlement, parsed ?? undefined)
      : false;
    ...
    return {
      ...
      dealFlags: (hasDataConflict || hasComplexDeal || hasAmountMismatch)
        ? { hasConflict: hasDataConflict, needsReview: hasComplexDeal, amountMismatch: hasAmountMismatch }
        : null,
    };
  });

  const needsReviewCount = serialized.filter(
    (r) =>
      r.dealFlags?.hasConflict ||
      r.dealFlags?.needsReview ||
      r.dealFlags?.amountMismatch ||
      r.settlement?.status === "disputed" ||
      r.settlement?.status === "revised"
  ).length;
  ...
  return (
    <div className="px-12 py-10 max-w-7xl">
      ...
      <div className="grid grid-cols-5 gap-px bg-ink-200/40 rounded-xl overflow-hidden mb-14">
        <StatCard label="Shows" value={String(reversed.length)} />
        <StatCard label="Needs review" value={String(needsReviewCount)} warn />
        <StatCard label="Disputed" value={String(disputedCount)} danger />
