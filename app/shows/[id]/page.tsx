...
export default async function ShowDetailPage({
  ...
  const parsed = deal?.dealNotesFreetext
    ...
    : null;
  const dealWarnings =
    parsed && deal ? generateWarnings(parsed, deal, expenses) : [];
  ...
  return (
    <div className="max-w-7xl">
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-4">
              ...
              {dealWarnings.length > 0 && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10.5px] font-medium ring-1 ring-inset bg-amber-50 text-amber-800 ring-amber-200/80">
                  <AlertTriangle className="h-3 w-3" />
                  {dealWarnings.length} {dealWarnings.length === 1 ? "issue" : "issues"}
                </span>
              )}
              {bonuses.length > 0 && (
      ...
      <div className="px-12 pb-12">
        ...
        {dealWarnings.length > 0 && (
          <div className="mb-8 mt-1 rounded-lg bg-amber-50/60 ring-1 ring-amber-300/60 p-5 flex gap-3">
            <AlertTriangle className="h-4 w-4 text-amber-700 mt-0.5 shrink-0" />
            <div className="flex-1">
              <div className="text-[13px] font-semibold text-amber-900">
                {dealWarnings.length} data{" "}
                {dealWarnings.length === 1 ? "issue" : "issues"} detected in
                deal terms
              </div>
              <div className="flex flex-wrap gap-1.5 mt-2">
                {dealWarnings.map((w, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center px-2 py-0.5 rounded text-[10.5px] font-medium bg-amber-100/80 text-amber-800 ring-1 ring-amber-200/50"
                  >
                    {w.message}
                  </span>
                ))}
              ...
              <Link
                ...
                className="inline-flex items-center gap-1 text-[12px] text-amber-800 font-medium hover:text-amber-900 mt-3 transition-colors"
              >
                Review in settlement &rarr;
              </Link>
            </div>
          </div>
        )}
                  ...
                  {bonuses.length > 0 && (
                    <div className="rounded-lg ring-1 ring-brand-200/50 bg-brand-50/20 p-4">
                      ...
                      <div className="text-[11px] text-ink-400 mt-3 leading-snug">
                        Check the deal notes below for any additional bonus
