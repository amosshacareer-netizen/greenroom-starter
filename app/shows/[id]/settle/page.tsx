...
import { parseDeal, generateWarnings } from "@/lib/parseDeal";
import type { ParsedDeal, Warning } from "@/lib/parseDeal";
import {
  formatMoney,
  formatShowDateFull,
} from "@/lib/format";
...
export default async function SettlePage({
  ...
  const parsed = deal.dealNotesFreetext
    ? parseDeal(deal.dealNotesFreetext)
    : null;
  const warnings = parsed
    ? generateWarnings(parsed, deal, expenses)
    : [];
  const needsManualReview = warnings.some(
    (w) => w.type === "complex_deal"
  );
  ...
  return (
    <div className={`px-12 py-10 max-w-7xl ${isDisputed ? "bg-gradient-to-b from-rose-50/30 via-canvas to-canvas" : ""}`}>
      ...
      <div className="space-y-6 mt-6">
        {parsed && (
          <DealTermsCard parsed={parsed} freetext={deal.dealNotesFreetext!} />
        )}
        ...
        {calc.supported ? (
          <SupportedSettlement calc={calc} existingSettlement={settlement} needsManualReview={needsManualReview} />
        ) : (
          <UnsupportedDeal
    ...
    <>
      ...
      <div className="text-center py-10 mb-2">
        <div className="eyebrow text-[10px] text-ink-400 mb-3">
          {existingSettlement?.status === "voided" ? "Settlement voided" : "Total to artist"}
        </div>
        <div
          className={`text-[72px] font-mono tabular font-bold leading-none ${
        ...
        {existingSettlement?.status === "voided" && (
          <div className="text-[13px] text-ink-400 mt-3">
            This show was cancelled. No payment was made.
          </div>
        )}
        {existingSettlement && existingSettlement.status !== "voided" && (
          <div className="mt-3">
        ...
        {needsManualReview && (
          <div className="mt-3">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-medium bg-amber-50 text-amber-800 ring-1 ring-inset ring-amber-200/80">
              <AlertTriangle className="h-3 w-3" />
      ...
      {/* Amount verification */}
      {existingSettlement && existingSettlement.totalToArtist != null && existingSettlement.status !== "voided" && (
        (() => {
          const recorded = existingSettlement.totalToArtist!;
          const calculated = calc.totalToArtist;
          const diff = Math.abs(recorded - calculated);
          const matches = diff < 0.01;
          const isPaid = existingSettlement.status === "paid";

          if (matches) {
            return (
              <div className="rounded-lg border border-brand-200/60 bg-brand-50/30 px-5 py-4 flex items-center gap-3 mb-6">
                <CheckCircle2 className="h-4 w-4 text-brand-700 shrink-0" />
                <div className="text-[13px] text-brand-900">
                  <span className="font-medium">Calculation matches {isPaid ? "paid" : "recorded"} amount</span>
                  <span className="text-brand-700 ml-1">
                    — {formatMoney(recorded)} {isPaid ? "was wired to artist" : "on record"}
                  </span>
                </div>
              </div>
            );
          ...
          return (
            <div className="rounded-lg border border-amber-200/60 bg-amber-50/40 px-5 py-4 mb-6">
              <div className="flex items-start gap-3">
                <AlertTriangle className="h-4 w-4 text-amber-700 mt-0.5 shrink-0" />
                <div>
                  <div className="text-[13px] font-semibold text-amber-900">
                    {isPaid ? "Paid amount" : "Recorded amount"} differs from calculation
                  </div>
                  <div className="mt-2 grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-[10px] font-medium text-ink-400 uppercase tracking-wider mb-1">
                        {isPaid ? "Actually paid" : "On record"}
                      </div>
                      <div className="text-[20px] font-mono tabular font-semibold text-ink-900">
                    ...
                    <div>
                      ...
                      <div className="text-[20px] font-mono tabular font-semibold text-amber-800">
                        {formatMoney(calculated)}
                      </div>
                    </div>
                  </div>
                  <div className="text-[12px] text-ink-500 mt-2">
                    Difference: <span className="font-mono tabular font-medium text-amber-800">{formatMoney(diff)}</span>
                    {isPaid && " — payment has already been sent at the recorded amount"}
                  </div>
                </div>
              </div>
            </div>
      ...
      <Card accent="brand">
        ...
        <CardContent className="divide-y divide-ink-100/80">
          <Row
            ...
            source="ticket_sales"
          />
          <Row label="Net box office" value={formatMoney(calc.netBoxOffice)} source="ticket_sales" />
          <Row
            label="Total expenses (passed through)"
            value={formatMoney(calc.totalExpenses)}
            source="expenses"
          />
          {calc.expenseBreakdown.map((e, i) => (
            <Row
              key={`exp-${i}`}
          ...
          <div className="flex items-baseline justify-between py-3 font-semibold">
            <span className="text-[14px] text-ink-900 uppercase tracking-wide">
              Artist takes
              {needsManualReview && (
                <span className="text-[10px] text-amber-700 ml-2 normal-case tracking-normal font-medium">(approximate)</span>
              )}
            </span>
            <span className={`text-[24px] font-mono tabular font-bold ${needsManualReview ? "text-amber-900" : "text-ink-900"}`}>
              {needsManualReview && "≈ "}{formatMoney(calc.totalToArtist)}
            </span>
          </div>
        </CardContent>
      </Card>
    ...
    <Card>
      <CardHeader>
        <div>
          ...
          <CardDescription>
            Extracted from the deal notes below.
          </CardDescription>
        </div>
        <PlainBadge variant="brand">From deal notes</PlainBadge>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-3">
            <Field label="Type" value={parsed.dealType ? (friendlyType[parsed.dealType] ?? parsed.dealType) : "Could not determine from notes"} />
            {parsed.guarantee != null && (
              <Field label="Guarantee" mono value={formatMoney(parsed.guarantee)} />
            )}
            {parsed.percentage != null && (
              <Field
                label="Percentage"
                mono
                value={`${(parsed.percentage * 100).toFixed(0)}%`}
              />
            )}
            {parsed.percentageBasis && (
              <Field label="Basis" value={parsed.percentageBasis} />
            )}
            {parsed.expenseCap != null && (
              <Field label="Expense cap" mono value={formatMoney(parsed.expenseCap)} />
            )}
            {parsed.hospitalityCap != null && (
              <Field label="Hospitality cap" mono value={formatMoney(parsed.hospitalityCap)} />
            )}
          </div>
          <div>
            <div className="eyebrow text-[10px] text-ink-500 mb-2">
            ...
            <div className="text-[12.5px] text-ink-600 bg-canvas-soft rounded-lg p-4 ring-1 ring-ink-200/60 leading-relaxed italic">
...
function Row({
  ...
  return (
    <div className={`flex items-baseline justify-between py-2.5 ${indent ? "pl-6" : ""}`}>
      <div className="min-w-0">
        <div className={`text-[13px] ${indent ? "text-ink-400" : "text-ink-600"}`}>{label}</div>
        {note && (
      ...
      <div className="flex items-baseline gap-3 shrink-0">
        ...
        {source && (
          <div className="text-[10px] text-ink-300 font-mono w-[70px] text-right">
