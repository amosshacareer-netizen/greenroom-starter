...
import { ArrowUpRight, Search, Calendar, AlertTriangle } from "lucide-react";
...
> = {
  ...
  revised: { variant: "sky", label: "In review" },
  finalized: { variant: "brand", label: "Signed" },
  paid: { variant: "brand", label: "Paid" },
  voided: { variant: "default", label: "Voided" },
};

function rowNeedsReview(row: ShowRow): boolean {
  return !!(
    row.dealFlags?.hasConflict ||
    row.dealFlags?.needsReview ||
    row.dealFlags?.amountMismatch ||
    row.settlement?.status === "disputed" ||
    row.settlement?.status === "revised"
  );
}
...
function getAccentColor(row: ShowRow): string {
  if (row.settlement) {
    ...
    if (s === "revised" || s === "submitted" || s === "in_review") return "bg-sky-400";
    return "bg-ink-300";
  }
  return "bg-ink-200";
}
...
export function ShowsList({ rows }: { rows: ShowRow[] }) {
  ...
  const reviewCount = useMemo(
    () => rows.filter(rowNeedsReview).length,
    [rows]
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "needs_review") {
      result = result.filter(rowNeedsReview);
    }
        ...
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium transition-colors ${
            filter === "needs_review"
              ...
              : "text-amber-700 hover:bg-amber-50 hover:ring-1 hover:ring-amber-200/60"
          }`}
        >
    ...
    <li className="relative group list-none">
      ...
      <Link
        ...
        className={`grid grid-cols-[84px_1fr_120px_auto_24px] items-center gap-4 pl-5 pr-2 py-3 rounded-lg hover:bg-white/80 hover:shadow-[0_1px_4px_rgba(26,24,20,0.04)] transition-all duration-150 ${
          rowNeedsReview(row) ? "bg-amber-50/40" : ""
        }`}
      >
        <div>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-[14.5px] font-medium text-ink-900 truncate group-hover:text-brand-800 transition-colors">
              {artist?.name ?? "—"}
            </span>
            {rowNeedsReview(row) && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium bg-amber-100/80 text-amber-800 ring-1 ring-amber-300/60 shrink-0">
                <AlertTriangle className="h-3 w-3" />
                Needs review
              </span>
            )}
